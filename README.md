...
```
dab_project_101        <- Root directory. Both monorepo and polyrepo are supported.
│
├── dab_project_101       <- Contains python code, notebooks and ML resources related to one ML project. 
│   │
...
* Run `databricks configure --profile dab_project_101-dev --token --host <your-dev-workspace-url>`, passing the URL of your dev workspace.
...
* From within the root directory of the current project, use the [dbx sync](https://dbx.readthedocs.io/en/latest/guides/python/devloop/mixed/#using-dbx-sync-repo-for-local-to-repo-synchronization) tool to copy code files from your local machine into the Repo by running
  `dbx sync repo --profile dab_project_101-dev --source . --dest-repo your-repo-name`, where `your-repo-name` should be the last segment of the full repo name (`/Repos/username/your-repo-name`)
